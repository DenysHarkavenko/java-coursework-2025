...
public class AppointmentSlotController {
    ...
    @GetMapping("/{slotId}")
    @Operation(summary = "Get slot by ID", description = "Retrieves a specific appointment slot by its ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Appointment slot found"),
            @ApiResponse(responseCode = "404", description = "Appointment slot not found")
    })
    public AppointmentSlotDTO getSlotById(
            @Parameter(description = "ID of the appointment slot") @PathVariable UUID slotId) {
        return appointmentSlotService.getSlotById(slotId);
    }
